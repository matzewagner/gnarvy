...
export default class TestScope {
  ...
  async run() {
    ...
    const start = new Date
		...
		dateStr = date.toISOString().slice(-24).replace(/D/g,)
		logStr = `${dateStr.slice(0, 8)}_${time.replace(/:/g, )}.log`
    const startStr = `Cavy test suite started at ${start}.`
		const logger = fs.createWriteStream(`./IntegrationTestLogs/${logStr}`)
    ...
    for (let i = 0; i < this.testCases.length; i++) {
      ...
      try {
        ...
        const passStr = `${description}  ✅`
				console.log(passStr)
				logger.write(`n${passStr}`)
      } catch (e) {
        const failStr =`${description}  ❌
   			${e.message}`
				console.log(failStr)
				logger.write(`n${failStr}`)
      }
    ...
    const endStr = `Cavy test suite stopped at ${stop}, duration: ${duration} seconds.`
		console.log(endStr)
		logger.write(`n${endStr}`)
		logger.end()
  }
